...
import in.ac.prsu.mca.mca2025_tictactoeapi.constants.Constants.PLAYER;
...
public class Test {
	...
	// supposed to called after updating pseudo state, to test whether that will
	// result in WIN in NEXT INN.
	public static boolean willIWonInNextState(Constants.PLAYER player, Game game, Position position) {
		...
		final boolean[] winningConditions = { winningCondition1, winningCondition2, winningCondition3,
				winningCondition4 };
		...
		for (boolean wc : winningConditions) {
			if (wc)
				numberOfWinningConditions++;
		}

		return numberOfWinningConditions >= 2;
	}

	// supposed to called after updating pseudo or real state, to test whether
	// playerID WON
	public static boolean won(Game game, Constants.PLAYER player, Position p) {
		...
		if (player == PLAYER.AI) {
			return (rowSum == Constants.PLAYER_AI_WINNING_SUM || colSum == Constants.PLAYER_AI_WINNING_SUM
					|| diagonalSum.getSumOfD1() == Constants.PLAYER_AI_WINNING_SUM
					|| diagonalSum.getSumOfD2() == Constants.PLAYER_AI_WINNING_SUM);
		} else {
			return (rowSum == Constants.PLAYER_HUMAN_WINNING_SUM || colSum == Constants.PLAYER_HUMAN_WINNING_SUM
	...
	public static int numberOfOpenPostions(Constants.PLAYER player, Game game, Position position) {
		...
		final boolean[] openGroups = { rowIsOpen, columIsOpen, diagonal1isOpen, diagonal2isOpen };
	...
	public static boolean diagonalDangerForAi(Game game) {
		final DiagonalSum diagonalSum = game.getDiagonalSum();
		return game.state[1][1] == Constants.PLAYER_AI_MARK && game.getNumberOfCellUpdated() == 3
				&& (diagonalSum.getSumOfD1() == Constants.DOUBLE_DANGER_SUM_FOR_AI
